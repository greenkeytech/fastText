...
fasttext_src_cc = list(map(lambda x: str(os.path.join(FASTTEXT_SRC, x)), fasttext_src_cc))
  ...
  def build_extensions(self):
    if sys.platform == 'darwin':
      ...
      if has_flag(self.compiler, [all_flags[0]]):
        ...
        raise RuntimeError(
          'libc++ is needed! Failed to compile with {} and {}.'.format(" ".join(all_flags), all_flags[0])
        )
