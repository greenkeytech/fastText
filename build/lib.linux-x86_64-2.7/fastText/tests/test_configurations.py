...
def check_supervised_configurations(configurations, verbose=1):
  for i in range(len(configurations)):
    configurations[i] = check_supervised_configuration(configurations[i], verbose=verbose)
...
def get_supervised_models(thread=None, verbose=1):
  if thread is None:
    thread = max_thread()
  sup_job_dataset = [
    "ag_news", "sogou_news", "dbpedia", "yelp_review_polarity", "yelp_review_full", "yahoo_answers",
  ...
  sup_job_size = [405607193, 421445471, 447481878, 427867393, 431292576, 517549567, 483742593, 493604598]
  ...
  sup_job_quant_size = [1600000, 1500000, 1700000, 1600000, 1600000, 1700000, 1600000, 1600000]
  ...
  configurations = check_supervised_configurations(configurations, verbose=verbose)
  return configurations
