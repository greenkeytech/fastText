...
def get_random_data(
  ...
  random_words = get_random_words(max_vocab_size, min_len_word, max_len_word, unique=unique_words)
  lines = []
...
class TestFastTextUnitPy(unittest.TestCase):
  ...
  def gen_test_supervised_util_test(self, kwargs):
    ...
    def check(data):
      ...
      with tempfile.NamedTemporaryFile(delete=False) as tmpf, tempfile.NamedTemporaryFile(delete=False) as tmpf2:
        for line in train_data:
          tmpf.write(("__label__" + line.strip() + "\n").encode("UTF-8"))
        tmpf.flush()
        for line in valid_data:
          tmpf2.write(("__label__" + line.strip() + "\n").encode("UTF-8"))
        tmpf2.flush()
        model = train_supervised(input=tmpf.name, **kwargs)
        true_labels = []
        all_words = []
  ...
  def gen_test_supervised_multiline_predict(self, kwargs):
    ...
    check_predict(build_supervised_model(get_random_data(100, min_words_line=1), kwargs))
  ...
  def gen_test_supervised_exercise_is_quant(self, kwargs):
    f = build_supervised_model(get_random_data(1000, max_vocab_size=1000), kwargs)
...
def gen_sup_test(configuration, data_dir):
  ...
  def sup_test(self):
    ...
    def get_path_size(path):
      path_size = subprocess.check_output(["stat", "-c", "%s", path]).decode('utf-8')
    ...
    def check(model, model_filename, test, lessthan, msg_prefix=""):
      ...
      self.assertEqual(N_local_out, test["n"], msg_prefix + "N: Want: " + str(test["n"]) + " Is: " + str(N_local_out))
      self.assertTrue(
        p1_local_out >= test["p1"], msg_prefix + "p1: Want: " + str(test["p1"]) + " Is: " + str(p1_local_out)
      )
      self.assertTrue(
        r1_local_out >= test["r1"], msg_prefix + "r1: Want: " + str(test["r1"]) + " Is: " + str(r1_local_out)
      )
      path_size = get_path_size(model_filename)
      size_msg = str(test["size"]) + " Is: " + str(path_size)
      if lessthan:
        self.assertTrue(path_size <= test["size"], msg_prefix + "Size: Want at most: " + size_msg)
      else:
        self.assertTrue(path_size == test["size"], msg_prefix + "Size: Want: " + size_msg)

    configuration["args"]["input"] = os.path.join(data_dir, configuration["args"]["input"])
    configuration["quant_args"]["input"] = configuration["args"]["input"]
    configuration["test"]["data"] = os.path.join(data_dir, configuration["test"]["data"])
    ...
    check(model, output + ".bin", configuration["test"], False, msg_prefix="Supervised: ")
    print()
    model.quantize(**configuration["quant_args"])
    model.save_model(output + ".ftz")
    check(model, output + ".ftz", configuration["quant_test"], True, msg_prefix="Quantized: ")
...
def gen_unit_tests(verbose=0):
  gen_funcs = [
    func for func in dir(TestFastTextUnitPy) if callable(getattr(TestFastTextUnitPy, func))
  ...
  for gen_func in gen_funcs:
    ...
    def build_test(test_name, kwargs=None):
      ...
      def test(self):
        return getattr(TestFastTextUnitPy, "gen_" + test_name)(self, copy.deepcopy(kwargs))
    ...
    if "_unsupervised_" in test_name:
      for i, setting in enumerate(unsupervised_settings):
        setattr(TestFastTextUnitPy, test_name + "_" + str(i), build_test(test_name, setting))
...
def gen_tests(data_dir, verbose=1):
  ...
  for configuration in get_supervised_models(verbose=verbose):
    setattr(
      TestFastTextPy, "test_sup_" + str(i) + "_" + configuration["dataset"], gen_sup_test(configuration, data_dir)
    )
    i += 1
