...
def process_question(question, cossims, model, words, vectors):
  ...
  for line in question:
    ...
    if words[util.find_nearest_neighbor(query, vectors, ban_set, cossims=cossims)] == qwords[3]:
      correct += 1
    num_qs += 1
...
def print_compute_accuracy_score(question, correct, num_qs, total_accuracy, semantic_accuracy, syntactic_accuracy):
  print(
    (
...
if __name__ == "__main__":
  ...
  for question in questions:
    ...
    quads = quads[1:-1]
    correct, num_qs, num_lines = process_question(quads, cossims, f, words, vectors)
    total_qs += num_qs
  ...
  print(
    "Questions seen / total: {0} {1}   {2:.2f} %".format(total_qs, total_num_lines, total_qs / total_num_lines * 100)
  )
